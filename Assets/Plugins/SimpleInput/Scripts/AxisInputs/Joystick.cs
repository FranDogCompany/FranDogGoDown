...
#pragma warning restore 0649
		...
		void Update()
		{
			...
			// if (xAxis.value > 0.5)
			// {
			// 	float speed = GameObject.Find("Player").GetComponent<Player>().moveSpeed;   //取得PlayerControler腳本上的speed值

			// 	playerIdle.transform.position += new Vector3(speed, 0, 0);
			// }
			// //類比搖桿_左移動
			// if (xAxis.value < -0.5)
            // {
			...
			// 	playerIdle.transform.position += new Vector3(0, speed, 0);
			// }
			// //類比搖桿_下移動
			// if (yAxis.value < -0.5)
			// {
